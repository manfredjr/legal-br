# Modelo — consulta a advogado externo

Nome do arquivo de saída: `CONSULTA-ADVOGADO-<assunto>-AAAA-MM-DD.md`

Objetivo: o advogado responde sem refazer a análise. Perguntas fechadas, contexto mínimo, apuração já feita.

---

# Consulta jurídica — [ASSUNTO]

| | |
|---|---|
| Solicitante | [PREENCHER] |
| Data | [AAAA-MM-DD] |
| Documento relacionado | `[ARQUIVO]` |
| Questões | [N] |
| Prazo desejado | [DATA ou "sem prazo definido"] |

## Contexto

[Cinco a dez linhas. O que é o produto/contrato, quem são as partes, o que se pretende fazer. Só o necessário para responder — não é briefing comercial.]

## O que já foi apurado

Levantamento feito pelo agente, com as fontes abertas e verificadas.

| Ponto | Conclusão | Fonte | Data |
|---|---|---|---|
| [PONTO] | [CONCLUSÃO] | [NORMA, ART. + URL/arquivo] | [AAAA-MM-DD] |

## Questões

Repita o bloco para cada questão.

### Questão 1 — [TÍTULO CURTO]

**Pergunta**

[Pergunta objetiva, respondível com sim/não ou com uma escolha entre alternativas. Evite "o que o senhor acha".]

**Por que o agente não concluiu**

[ ] Norma não localizada em fonte oficial
[ ] Mais de uma interpretação plausível
[ ] Exige juízo sobre validade no caso concreto
[ ] Tema fora do escopo do agente ([qual])
[ ] Litígio, notificação ou processo em curso
[ ] Outro: [DESCREVER]

**Interpretações possíveis**

| # | Interpretação | Fundamento | Risco se adotada |
|---|---|---|---|
| A | [DESCRIÇÃO] | [NORMA/FONTE] | [RISCO] |
| B | [DESCRIÇÃO] | [NORMA/FONTE] | [RISCO] |

**Impacto prático da resposta**

[O que muda no produto, no contrato ou na operação conforme a resposta. Seja concreto: campo de cadastro, cláusula, fluxo, prazo de retenção.]

**Urgência:** Alta | Média | Baixa — [justificativa em uma linha]

**Trecho relacionado**

> [TRANSCRIÇÃO DA CLÁUSULA OU DO PONTO EM DISCUSSÃO]

---

## Anexos

- `[ARQUIVO]` — [descrição]

## Resposta do advogado

Espaço reservado. A ser preenchido pelo profissional.

### Questão 1

- **Parecer:**
- **Fundamento:**
- **Ressalvas:**
- **Advogado:** [NOME] — OAB [UF] [NÚMERO]
- **Data:**

---

[COLAR AQUI O BLOCO DE templates/aviso-agente-ia.md]
